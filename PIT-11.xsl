<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tns="http://crd.gov.pl/wzor/2022/11/09/11890/" version="1.0">
	...
	<xsl:template name="MiejsceICel">
		...
		<h2 class="tytul-sekcja-blok">
			<xsl:value-of select="$sekcja"/> MIEJSCE I CEL SKŁADANIA INFORMACJI
    </h2>
				...
				<td class="wypelniane">
					<xsl:choose>
						<xsl:when test="*[local-name()='Naglowek']/*[local-name()='CelZlozenia'] = 1">
							<input type="checkbox" checked="checked" disabled="disabled"/>1. złożenie informacji
            </xsl:when>
						<xsl:when test="*[local-name()='Naglowek']/*[local-name()='CelZlozenia'] = 2">
							<input type="checkbox" checked="checked" disabled="disabled"/>2. korekta informacji<sup>8)</sup>
						</xsl:when>
					</xsl:choose>
				</td>
			</tr>
	...
	<xsl:template name="Skladajacy">
		<xsl:param name="sekcja"/>
		<h2 class="tytul-sekcja-blok">
			<xsl:value-of select="$sekcja"/> DANE IDENTYFIKACYJNE SKŁADAJĄCEGO<br/>
			...
			<xsl:if test="count(tns:Podmiot1/*[local-name()='OsobaNiefizyczna']) &gt; 0">
				<xsl:for-each select="tns:Podmiot1/*[local-name()='OsobaNiefizyczna']">
					...
					<tr>
						<td class="wypelniane" style="width: 100%">
							<div class="opisrubryki">8. Rodzaj składającego</div>
							<input type="checkbox" checked="checked" disabled="disabled"/>1. składający niebędący osobą fizyczną
            </td>
					</tr>
					<tr>
						<td class="wypelniane">
	...
	<xsl:template name="DaneIdentyfikacyjne">
		<xsl:param name="sekcja"/>
		<h2 class="tytul-sekcja-blok">
			<xsl:value-of select="$sekcja"/> DANE IDENTYFIKACYJNE I ADRES ZAMIESZKANIA PODATNIKA
    </h2>
					...
					<td class="wypelniane" style="width: 100%">
						...
						<xsl:choose>
							<xsl:when test="tns:P_11 =1">
								<input type="checkbox" checked="checked" disabled="disabled"/>1. nieograniczony
                obowiązek podatkowy (rezydent)
              </xsl:when>
							<xsl:when test="tns:P_11 =2">
								<input type="checkbox" checked="checked" disabled="disabled"/>2. ograniczony obowiązek podatkowy (nierezydent)<sup>10)</sup>
							</xsl:when>
						</xsl:choose>
					</td>
				</tr>
							...
							<xsl:for-each select="*[local-name()='OsobaFizyczna']">
								<xsl:choose>
									<xsl:when test="//tns:RodzajNrId=1">
										<input type="checkbox" checked="checked" disabled="disabled"/>1. numer
                    identyfikacyjny
                    TIN
                  </xsl:when>
									<xsl:when test="//tns:RodzajNrId=2">
										<input type="checkbox" checked="checked" disabled="disabled"/>2. numer
                    ubezpieczeniowy
                  </xsl:when>
									<xsl:when test="//tns:RodzajNrId= 3">
										<input type="checkbox" checked="checked" disabled="disabled"/>3. paszport
                  </xsl:when>
									<xsl:when test="//tns:RodzajNrId=4">
										<input type="checkbox" checked="checked" disabled="disabled"/>4. urzędowy
                    dokument
                    stwierdzający tożsamość
                  </xsl:when>
									<xsl:when test="//tns:RodzajNrId=8">
										<input type="checkbox" checked="checked" disabled="disabled"/>8. inny rodzaj
                    identyfikacji podatkowej
                  </xsl:when>
									<xsl:when test="//tns:RodzajNrId=9">
										<input type="checkbox" checked="checked" disabled="disabled"/>9. inny dokument
	...
	<xsl:template name="KosztyUzyskaniaPrzychodu">
		...
		<xsl:for-each select="tns:PozycjeSzczegolowe">
			<h2 class="tytul-sekcja-blok">
				<xsl:value-of select="$sekcja"/> INFORMACJA O KOSZTACH UZYSKANIA PRZYCHODU
              ...
              uwzględnione do wysokości przysługującej podatnikowi
						...
						<xsl:choose>
							<xsl:when test="//tns:P_28 = 1">
								<input type="checkbox" checked="checked" disabled="disabled"/>1. z jednego stosunku pracy (stosunków pokrewnych)
              </xsl:when>
							<xsl:when test="//tns:P_28 = 2">
								<input type="checkbox" checked="checked" disabled="disabled"/>2. z więcej niż jednego stosunku pracy (stosunków pokrewnych)
              </xsl:when>
							<xsl:when test="//tns:P_28 = 3">
								<input type="checkbox" checked="checked" disabled="disabled"/>3. z jednego stosunku pracy (stosunków pokrewnych), podwyższone w związku z zamieszkiwaniem podatnika poza miejscowością, w której znajduje się zakład pracy
              </xsl:when>
							<xsl:when test="//tns:P_28 = 4">
								<input type="checkbox" checked="checked" disabled="disabled"/>4. z więcej niż jednego stosunku pracy (stosunków pokrewnych), podwyższone w związku z zamieszkiwaniem podatnika poza miejscowością, w której znajduje się zakład pracy
              </xsl:when>
						</xsl:choose>
					</td>
				</tr>
	...
	<xsl:template name="DochodyPodatnika">
		...
		<xsl:for-each select="tns:PozycjeSzczegolowe">
			<h2 class="tytul-sekcja-blok">
				<xsl:value-of select="$sekcja"/> DOCHODY PODATNIKA, POBRANE ZALICZKI ORAZ POBRANE SKŁADKI NA UBEZPIECZENIA SPOŁECZNE
	...
	<xsl:template name="InformacjaOPrzychodach1">
		...
		<xsl:for-each select="tns:PozycjeSzczegolowe">
			<div class="tytul-sekcja-blok">
				<h2>
					<xsl:value-of select="$sekcja"/> INFORMACJA O WYSOKOŚCI PRZYCHODÓW, O KTÓRYCH MOWA W ART. 20 UST. 1 USTAWY
          <div class="opis-tekstowy">
	...
	<xsl:template name="InformacjaOPrzychodach2">
		...
		<xsl:for-each select="tns:PozycjeSzczegolowe">
			<div class="tytul-sekcja-blok">
				<h2>
					<xsl:value-of select="$sekcja"/> INFORMACJA O PRZYCHODACH ZWOLNIONYCH OD PODATKU, ZAŁĄCZNIKU ORAZ O POBRANYCH PRZEZ PŁATNIKA SKŁADKACH: NA UBEZPIECZENIE ZDROWOTNE ORAZ CZŁONKOWSKICH NA RZECZ ZWIĄZKÓW ZAWODOWYCH
					 <div class="opis-tekstowy">
            (część tę wypełniają składający będący płatnikami)
          </div>
				</h2>
            ...
            <div class="opis-tekstowy">Należy wykazać kwotę stypendium, do wysokości limitu zwolnienia<sup>21)</sup>.</div>
					</td>
					<td class="wypelniane" style="width:25%">
            ...
            Przychody ze stosunku pracy i stosunków pokrewnych, z umów zlecenia, praktyk absolwenckich lub staży uczniowskich oraz z zasiłków macierzyńskich, o których mowa w art. 21 ust. 1 pkt 148 ustawy<font style="font-weight: normal">
							<div class="opis-tekstowy">Należy wykazać kwotę przychodu, do wysokości limitu zwolnienia<sup>22)</sup>.</div>
						</font>
					</td>
					<td class="wypelniane" style="width:25%">
			...
			<table class="normalna">
				<tr>
					<td class="wypelniane" style="width:25%" colspan="2">
						...
						<xsl:choose>
							<xsl:when test="tns:P_118 = 1">118. <input type="checkbox" checked="checked" disabled="disabled"/>art. 21 ust. 1 pkt 152 ustawy<br/>
							</xsl:when>
							<xsl:otherwise>
								118. <input type="checkbox" disabled="disabled"/> art. 21 ust. 1 pkt 152 ustawy<br/>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="tns:P_119 = 1">119. <input type="checkbox" checked="checked" disabled="disabled"/> art. 21 ust. 1 pkt 153 ustawy<br/>
							</xsl:when>
							<xsl:otherwise>
								119. <input type="checkbox" disabled="disabled"/> art. 21 ust. 1 pkt 153 ustawy<br/>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:choose>
							<xsl:when test="tns:P_120 = 1">120. <input type="checkbox" checked="checked" disabled="disabled"/> art. 21 ust. 1 pkt 154 ustawy<br/>
							</xsl:when>
							<xsl:otherwise>
								120. <input type="checkbox" disabled="disabled"/> art. 21 ust. 1 pkt 154 ustawy
							</xsl:otherwise>
						</xsl:choose>
					</td>
				</tr>
